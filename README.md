...
- **Log entries**: date (defaults to today), task, optional description, and time worked entered as separate **hours** and **minutes** fields (e.g. `7` h `30` m) — not a clock/time-of-day picker.
...
- **Predefined description prompts**: configure Task, Unit and Name fragments and use them to compose a natural sentence which is appended to the Description field (the Description remains editable).
...
- **Download**: export everything as **CSV** (Excel-friendly). Hours are kept in the database but excluded from CSV exports.
